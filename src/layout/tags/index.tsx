...
import {
  ...
  clearOtherTags,
  clearALLTags,
} from "src/redux-store/tags/action";
...
const Tags = (props: IProps): ReactElement => {
  ...
  const items: MenuProps["items"] = [
    {
      key: "1",
      label: <span onClick={delOtherTags}>删除其他</span>,
    },
    {
      key: "2",
      label: <span onClick={delAllTags}>删除全部</span>,
    },
  ];
  return (
    <>
      <div className={styles["tags"]}>
        <div className="tag-items">
          {tagsList.map(
            (item: { path: string; label: string }, index: number) => {
              return (
                <Tag
                  key={item.path}
                  color={item.path === activeTag ? "processing" : "default"}
                  closable={index !== 0}
                  onClose={() => {
                    closeTags(item.path);
                  }}
